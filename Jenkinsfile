...
stages {
    ...
    stage('Check ZAP Report') {
        steps {
            script {
                def highCount = sh(script: "grep -o '<riskcode>3</riskcode>' zap-report/zap-report.xml | wc -l", returnStdout: true).trim()
                echo "[INFO] Обнаружено уязвимостей уровня High: ${highCount}"
                ...
                if (highCount.toInteger() > 0) {
                    withCredentials([
                        ...
                        sh '''
                            ...
                            -d chat_id=${TG_CHAT} \
                            -d text="[ALERT] Обнаружены уязвимости высокого уровня. Деплой прерван."
                        '''
                    }
                    error("ZAP обнаружил HIGH уязвимости — пайплайн остановлен.")
                }
            }
        }
    }
